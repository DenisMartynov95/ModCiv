...
------------------------------------------------------------------------------
--	MODDER:   GAIA
--	THANKS:	  WHoward, Barathor, HFF, azum4roll, adan_eslavo, llamiran
------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
--## GAIA MOD START
local bHasPseudoNaturalWonders = false
for row in DB.Query("PRAGMA table_info(Features)") do
    if row.name == "PseudoNaturalWonder" then
         bHasPseudoNaturalWonders = true
         break
    end
end
--## GAIA MOD END
...
local MAX_MAJOR_CIVS     = GameDefines.MAX_MAJOR_CIVS or 22
local MAX_MINOR_CIVS     = math.min((63 - MAX_MAJOR_CIVS), #GameInfo.MinorCivilizations)
...
function AssignStartingPlots.Create()
	...
	local findStarts = {
		...
		PlaceTulips = AssignStartingPlots.PlaceTulips,
		...
		PlaceKelp = AssignStartingPlots.PlaceKelp, -- GAIA
		PlaceMinerals = AssignStartingPlots.PlaceMinerals, -- GAIA
		...
		iNumMaxAllowedForRegions = 18,	-- Maximum luxury types allowed to be assigned to regional distribution. CANNOT be reduced below 8!		-- MOD.Barathor: original = 8
		...
		iNumTypesSpecialCase = 2,		-- Marble affects Wonder, Tulips on Marshes
		...
		flood_plains_list = {},							-- 3
		hills_open_list = {},							-- 4
		hills_covered_list = {},						-- 5
		hills_jungle_list = {},							-- 6
		hills_forest_list = {},							-- 7
		jungle_flat_list = {},							-- 8
		forest_flat_list = {},							-- 9
		desert_flat_no_feature = {},					-- 10
		plains_flat_no_feature = {},					-- 11
		dry_grass_flat_no_feature = {},					-- 12
		fresh_water_grass_flat_no_feature = {},			-- 13
		tundra_flat_including_forests = {},				-- 14
		forest_flat_that_are_not_tundra = {},			-- 15
		grass_flat_no_feature = {},						-- 16
		tundra_flat_no_feature = {},					-- 17
		dry_plains_flat_no_feature = {},				-- 18
		fresh_water_plains_flat_no_feature = {},		-- 19
		desert_or_tundra_flat_no_feature = {},			-- 20
		tundra_flat_forest = {},						-- 21
		hills_open_no_tundra = {},						-- 22
		hills_open_no_desert = {},						-- 23
		hills_open_no_tundra_no_desert = {},			-- 24
		hills_open_no_grass = {},						-- 25
		hills_open_no_grass_no_tundra = {},				-- 26
		hills_open_no_grass_no_plains = {},				-- 27
		hills_covered_no_tundra = {},					-- 28
		hills_covered_no_grass = {},					-- 29
		hills_covered_no_grass_no_tundra = {},			-- 30
		flat_covered = {},								-- 31
		flat_covered_no_grass = {},						-- 32
		flat_covered_no_tundra = {},					-- 33
		flat_covered_no_grass_no_tundra = {},			-- 34
		flat_open = {},									-- 35
		flat_open_no_grass_no_plains = {},				-- 36
		flat_open_no_tundra_no_desert = {},				-- 37
		flat_open_no_desert = {},						-- 38
		flat_desert_including_flood = {},				-- 39
		tropical_marsh_list = {},						-- 40
		snow_flat_list = {},							-- 41
		hills_list = {},								-- 42
		land_list = {},									-- 43
		coast_list = {},								-- 44
		marble_list = {},								-- 45
		extra_deer_list = {},							-- 46
		desert_wheat_list = {},							-- 47
		banana_list = {},								-- 48
		coconut_list = {},								-- 49
		hills_open_tundra = {},							-- 50
		hills_open_grass = {},							-- 51
		hills_open_plains = {},							-- 52
		hills_open_desert = {},							-- 53
		hills_snow_list = {},							-- 54
		hills_forest_tundra = {},						-- 55
		land_hills_list = {},							-- 56
		mountain_list = {},								-- 57
		lake_list = {},									-- 58
		ice_list = {},									-- 59
		...
		-- Mod luxuries
		coffee_ID, tea_ID, tobacco_ID, amber_ID, jade_ID, olives_ID, coral_ID, lapis_ID, lavender_ID, poppies_ID,
		-- Mod bonus
		maize_ID, rice_ID, tin_ID, berries_ID, cactus_ID, coconuts_ID, flax_ID, rubber_ID, timber_ID, lakefish_ID, kelp_ID, minerals_ID,
...
function AssignStartingPlots:__Init()
	...
	for resource_data in GameInfo.Resources() do
		...
		elseif resourceType == "RESOURCE_GAIA_MAIZE" then
			self.maize_ID = resourceID;
		elseif resourceType == "RESOURCE_GAIA_RICE" then
		...
		-- Mod Luxury IDs
		elseif resourceType == "RESOURCE_GAIA_COFFEE" then
			self.coffee_ID = resourceID;
		elseif resourceType == "RESOURCE_GAIA_TEA" then
			self.tea_ID = resourceID;
		elseif resourceType == "RESOURCE_GAIA_TOBACCO" then
			self.tobacco_ID = resourceID;
		elseif resourceType == "RESOURCE_GAIA_AMBER" then
			self.amber_ID = resourceID;
		elseif resourceType == "RESOURCE_GAIA_JADE" then
			self.jade_ID = resourceID;
		elseif resourceType == "RESOURCE_GAIA_OLIVES" then
			self.olives_ID = resourceID;
		elseif resourceType == "RESOURCE_GAIA_CORAL" then
			self.coral_ID = resourceID;
		elseif resourceType == "RESOURCE_GAIA_LAPIS" then
			self.lapis_ID = resourceID;
		elseif resourceType == "RESOURCE_GAIA_LAVENDER" then
			self.lavender_ID = resourceID;
		elseif resourceType == "RESOURCE_GAIA_POPPIES" then
			self.poppies_ID = resourceID;
		-- Mod Bonus IDs
		elseif resourceType == "RESOURCE_GAIA_TIN" then
			self.tin_ID = resourceID;
		elseif resourceType == "RESOURCE_GAIA_BERRIES" then
			self.berries_ID = resourceID;
		elseif resourceType == "RESOURCE_GAIA_CACTUS" then
			self.cactus_ID = resourceID;
		elseif resourceType == "RESOURCE_GAIA_COCONUTS" then
			self.coconuts_ID = resourceID;
		elseif resourceType == "RESOURCE_GAIA_FLAX" then
			self.flax_ID = resourceID;
		elseif resourceType == "RESOURCE_GAIA_RUBBER" then
			self.rubber_ID = resourceID;
		elseif resourceType == "RESOURCE_GAIA_TIMBER" then
			self.timber_ID = resourceID;
		elseif resourceType == "RESOURCE_GAIA_LAKEFISH" then
			self.lakefish_ID = resourceID;
		elseif resourceType == "RESOURCE_GAIA_KELP" then
			self.kelp_ID = resourceID;
		elseif resourceType == "RESOURCE_GAIA_MINERALS" then
			self.minerals_ID = resourceID;
		end
	end
...
function AssignStartingPlots:GenerateRegions(args)
	...
	elseif self.method == RegionDivision.RECTANGULAR or self.method == RegionDivision.RECTANGULAR_SELF_DEFINED then
		...
		--## GAIA MOD START
		if iNumRelevantLandAreas == 0 then
			print("Cannot generate map with the given parameters. Try with a larger map or change the parameters!")
		end
		--## GAIA MOD END
...
function CivNeedsMountainStart(civType)
	if GameInfo.Civilization_Start_Prefer_Mountain then
...
function CivNeedsSnowStart(civType)
    if GameInfo.Civilization_Start_Prefer_Snow then
...
function AssignStartingPlots:AttemptToPlaceBonusResourceAtPlot(x, y, bAllowOasis)
	...
	elseif plotType == PlotTypes.PLOT_OCEAN then
		if terrainType == TerrainTypes.TERRAIN_COAST and featureType == FeatureTypes.NO_FEATURE then
			--## MOD GAIA START
			if plot:IsLake() == true then -- Place Lake Resources
				plot:SetResourceType(self.lakefish_ID, 1);
				print("Placed Trout.");
				self.amounts_of_resources_placed[self.lakefish_ID + 1] = self.amounts_of_resources_placed[self.lakefish_ID + 1] + 1;
				return true, false
			else
				-- Place Fish
...
function AssignStartingPlots:AttemptToPlaceForestAtGrassPlot(x, y)
	-- GAIA: place Forest (not Cows or Stone!) to boost production at heavy grass starts.
...
--						iNumPlains = iNumPlains + 1;
	...
	if innerHammerScore < 3 and earlyHammerScore < 6 then -- Add a forest to second ring.
		...
		for attempt = 1, 12 do
			...
			local plot = Map.GetPlot(searchX, searchY) -- GAIA: missing declaration
	...
	if nativeTwoFoodTiles == 0 and iNumFoodBonusNeeded < 3 then
		...
		for direction_index = 1, 6 do
			for plot_to_handle = 1, ripple_radius do
				local plot_adjustments, nextX, nextY -- GAIA: missing declaration
				if currentY / 2 > math.floor(currentY / 2) then
					plot_adjustments = odd[direction_index];
				else
					plot_adjustments = even[direction_index];
...
function AssignStartingPlots:ExaminePlotForNaturalWondersEligibility(x, y)
	...
	if fertility < 10 then
		return false, false;
	elseif fertility < 14 then
		return false, true;
	end
	return true, true;
end
...
function AssignStartingPlots:GenerateNaturalWondersCandidatePlotLists(target_number)
	...
	if not bHasPseudoNaturalWonders then
		self.iNumNW = math.min (self.iNumNW, 17)	--GAIA: new Natural Wonders require a custom DLL
	end
	...
	for row in GameInfo.Features() do
		local isWonder = row.NaturalWonder == true
		if bHasPseudoNaturalWonders then
			isWonder = isWonder or row.PseudoNaturalWonder == 1
		end
...
function AssignStartingPlots:PlaceNaturalWonders()	
	...
	local worldsizes = {
		[GameInfo.Worlds.WORLDSIZE_DUEL.ID] = 2, 		--/*2*/
		[GameInfo.Worlds.WORLDSIZE_TINY.ID] = 4,		--/*3*/
		[GameInfo.Worlds.WORLDSIZE_SMALL.ID] = 6,		--/*4*/
		[GameInfo.Worlds.WORLDSIZE_STANDARD.ID] = 8,	--/*5*/
		[GameInfo.Worlds.WORLDSIZE_LARGE.ID] = 12,		--/*6*/
		[GameInfo.Worlds.WORLDSIZE_HUGE.ID] = 16		--/*7*/
		}
...
function AssignStartingPlots:GenerateResourcePlotListsFromSpecificPlots(plotList)
	...
	local temp_shore_list, temp_marsh_list, temp_flood_plains_list = {}, {}, {};
	...
	local temp_coconut_list, temp_flax_list, temp_offshore_list  = {}, {}, {};
	...
	for loop, plotIndex in ipairs(plotList) do
			...
			if featureType ~= self.feature_atoll then
				...
				elseif terrainType == TerrainTypes.TERRAIN_COAST then
					...
					if plot:IsAdjacentToLand() then
						table.insert(temp_shore_list, plotIndex);
					else
						table.insert(temp_offshore_list, plotIndex);
					end
				else
		...
		elseif featureType == FeatureTypes.FEATURE_FLOOD_PLAINS then
			table.insert(temp_flax_list, plotIndex);
			table.insert(temp_land_list, plotIndex);
			table.insert(temp_land_hills_list, plotIndex);
		...
		elseif plotType == PlotTypes.PLOT_LAND then
			...
			if featureType == FeatureTypes.FEATURE_JUNGLE then
				...
				if terrainType == TerrainTypes.TERRAIN_PLAINS then			
					...
					if plot:IsCoastalLand() then
						table.insert(temp_coconut_list, plotIndex);
					end
				elseif terrainType == TerrainTypes.TERRAIN_GRASS then
					table.insert(temp_flat_covered_no_tundra, plotIndex);
					if plot:IsCoastalLand() then
	...
	return {
		GetShuffledCopyOfTable(temp_shore_list),							-- 1
		...
		GetShuffledCopyOfTable(temp_banana_list),							-- 49
		...
		GetShuffledCopyOfTable(temp_flax_list),								-- 65
		GetShuffledCopyOfTable(temp_offshore_list),							-- 66
...
function AssignStartingPlots:GenerateGlobalResourcePlotLists()
	...
	print("Generating resource plot lists from specific plots");
	local resourcePlotLists = self:GenerateResourcePlotListsFromSpecificPlots(plotList);
	self.shore_list				 				 = resourcePlotLists[1];
	...
	--
	self.flax_list								 = resourcePlotLists[65];
	self.offshore_list					 		 = resourcePlotLists[66];
	
	-- Unused, but keeping this here anyway just in case some mapscripts need it
	self.barren_plots = table.maxn(self.mountain_list) +
	...
	print("Coconut list size:", table.maxn(self.coconut_list));
	print("Flax list size:", table.maxn(self.flax_list));
	print("Lake list size:", table.maxn(self.lake_list));
	...
	self.global_luxury_plot_lists = {
		...
		self.grass_flat_no_feature,					-- 16
		self.tundra_flat_no_feature,				-- 17
		self.dry_plains_flat_no_feature,			-- 18
		self.fresh_water_plains_flat_no_feature,	-- 19
		self.desert_or_tundra_flat_no_feature,		-- 20
		self.tundra_flat_forest,					-- 21
		self.hills_open_no_tundra,					-- 22
		self.hills_open_no_desert,					-- 23
		self.hills_open_no_tundra_no_desert,		-- 24
		self.hills_open_no_grass,					-- 25
		self.hills_open_no_grass_no_tundra,			-- 26
		self.hills_open_no_grass_no_plains,			-- 27
		self.hills_covered_no_tundra,				-- 28
		self.hills_covered_no_grass,				-- 29
		self.hills_covered_no_grass_no_tundra,		-- 30
		self.flat_covered,							-- 31
		self.flat_covered_no_grass,					-- 32
		self.flat_covered_no_tundra,				-- 33
		self.flat_covered_no_grass_no_tundra,		-- 34
		self.flat_open,								-- 35
		self.flat_open_no_grass_no_plains,			-- 36
		self.flat_open_no_tundra_no_desert,			-- 37
		self.flat_open_no_desert,					-- 38
		self.flat_desert_including_flood,			-- 39
		--
...
function AssignStartingPlots:GetLuxuriesSplitCap()
	...
	local split_cap = 1;
	-- With a new regional luxury cap of 18, there's no need for a split cap higher than 2 to cover the maximum civ count of 22 (18 x 2 = 36)
	-- Split caps higher than 2 are not ideal, and are more random and uneven.
...
function AssignStartingPlots:AssignLuxuryRoles()
	...
	-- Assign Marble and Tulips to special casing.
	table.insert(self.resourceIDs_assigned_to_special_case, self.marble_ID);
	table.insert(self.resourceIDs_assigned_to_special_case, self.tulips_ID);
	--self.iNumTypesUnassigned = self.iNumTypesUnassigned - 1;	-- MOD.Barathor: This is no longer needed.
				...
				elseif wrapY == false and (nextY < 0 or nextY >= iH) then
					...
					if plot:GetResourceType(-1) == -1 then	
						...
						elseif plotType == PlotTypes.PLOT_HILLS or plotType == PlotTypes.PLOT_LAND then
							...
							elseif terrainType == TerrainTypes.TERRAIN_DESERT then
								allowed_luxuries[self.poppies_ID] = true
								allowed_luxuries[self.marble_ID] = true
							...
							elseif terrainType == TerrainTypes.TERRAIN_PLAINS then
								...
								--allowed_luxuries[self.roses_ID] = true
...
function AssignStartingPlots:GetIndicesForLuxuryType(resource_ID)
	...
	elseif resource_ID == self.marble_ID then
		primary = 45;
	-- Tulips
	elseif resource_ID == self.tulips_ID then
		primary = 2;
	-- Rock Luxuries
	...
	elseif resource_ID == self.tea_ID then							
		primary, secondary, tertiary, quaternary, quinary, senary = 16, 11, 33, 24, 28, 2;
	--elseif resource_ID == self.roses_ID then						
		--primary, secondary, tertiary, quaternary, quinary, senary = 16, 11, 33, 24, 28, 2;
	...
	elseif resource_ID == self.lavender_ID then
		primary, secondary, tertiary, quaternary, quinary, senary = 16, 11, 33, 24, 28, 2;
	elseif resource_ID == self.poppies_ID then
...
------------------------------------------------------------------------------
function AssignStartingPlots:PlaceTulips()
	if not self.tulips_ID then
		print("Tulips resource not defined; skipping placement.");
		return;
	end

	local tulips_already_placed = self.amounts_of_resources_placed[self.tulips_ID + 1];
	local tulip_target = math.ceil(self.iNumCivs * 0.75);
	if self.luxuryDensity == 1 then
		tulip_target = math.ceil(self.iNumCivs * 0.5);
	elseif self.luxuryDensity == 3 then
		tulip_target = math.ceil(self.iNumCivs * 1.0);
	end

	local iNumTulipsToPlace = math.max(2, tulip_target - tulips_already_placed);
	local iW, iH = Map.GetGridSize();
	local iNumLeftToPlace = iNumTulipsToPlace;
	local iNumPlots = table.maxn(self.marsh_list);
	if iNumPlots < 1 then
		print("No eligible plots available to place Tulips!");
		return;
	end

	-- Main loop
	for place_resource = 1, iNumTulipsToPlace do
		for _, plotIndex in ipairs(self.marsh_list) do
			local x = (plotIndex - 1) % iW;
			local y = (plotIndex - x - 1) / iW;
			local res_plot = Map.GetPlot(x, y)
			if res_plot:GetResourceType(-1) == -1 then -- No existing resource
				res_plot:SetResourceType(self.tulips_ID, 1);
				self.amounts_of_resources_placed[self.tulips_ID + 1] =
					self.amounts_of_resources_placed[self.tulips_ID + 1] + 1;

				print("-"); print("Placed Tulips randomly at Plot", x, y);
				self.totalLuxPlacedSoFar = self.totalLuxPlacedSoFar + 1;
				print("Total luxuries placed so far:", self.totalLuxPlacedSoFar);
				iNumLeftToPlace = iNumLeftToPlace - 1;
				print("Still need to place", iNumLeftToPlace, "more Tulips.");
				break;
			end
		end
	end
	...
	if iNumLeftToPlace > 0 then
		print("Failed to place", iNumLeftToPlace, "Tulips.");
	end
end
------------------------------------------------------------------------------
...
---[[ ## MOD GAIA START
function AssignStartingPlots:PlaceKelp(frequency, plot_list)
	-- This function places kelp at a controlled frequency.
	if plot_list == nil then
		print("No kelp was placed! - PlaceKelp");
		return
	end

	local iW, iH = Map.GetGridSize();
	local iNumTotalPlots = table.maxn(plot_list);
	local iNumKelpToPlace = math.ceil(iNumTotalPlots / frequency);

	local current_index = 1;
	for place_resource = 1, iNumKelpToPlace do
		local placed_this_res = false;
		if current_index <= iNumTotalPlots then
			for index_to_check = current_index, iNumTotalPlots do
				...
				local y = (plotIndex - x - 1) / iW;
				local res_plot = Map.GetPlot(x, y)
				if res_plot:GetResourceType(-1) == -1 and res_plot:GetFeatureType() == FeatureTypes.NO_FEATURE then
					res_plot:SetResourceType(self.kelp_ID, 1);
					placed_this_res = true;
					self.amounts_of_resources_placed[self.kelp_ID + 1] = self.amounts_of_resources_placed[self.kelp_ID + 1] + 1;
				end
			end
		end
	end
...
function AssignStartingPlots:PlaceMinerals(frequency, plot_list)
	-- This function places deep-ocean minerals at a controlled frequency.
	if plot_list == nil then
		print("No minerals were placed! - PlaceMinerals");
		return
	end

	local iW, iH = Map.GetGridSize();
	local iNumTotalPlots = table.maxn(plot_list);
	local iNumMineralsToPlace = math.ceil(iNumTotalPlots / frequency);

	local current_index = 1;
	for place_resource = 1, iNumMineralsToPlace do
		local placed_this_res = false;
		if current_index <= iNumTotalPlots then
			for index_to_check = current_index, iNumTotalPlots do
				if placed_this_res then
				...
				local res_plot = Map.GetPlot(x, y)
				if res_plot:GetResourceType(-1) == -1 and res_plot:GetFeatureType() == FeatureTypes.NO_FEATURE then
					res_plot:SetResourceType(self.minerals_ID, 1);
					placed_this_res = true;
					self.amounts_of_resources_placed[self.minerals_ID + 1] = self.amounts_of_resources_placed[self.minerals_ID + 1] + 1;
				end
			end
		end
	end
...
function AssignStartingPlots:PlaceSexyBonusAtCivStarts()
	...
	for region_number = 1, self.iNumCivs do
				...
				elseif wrapY == false and (nextY < 0 or nextY >= iH) then
					...
					if plot:GetResourceType(-1) == -1 and featureType ~= FeatureTypes.FEATURE_OASIS then -- No resource or Oasis here, safe to proceed.
						...
						if plotType == PlotTypes.PLOT_OCEAN then
							...
							-- ## GAIA MOD START
							else table.insert(trout_list, plotIndex);
							-- ## GAIA MOD END
							end
						end
					end
		...
		if iNumCandidates > 0 then
			...
			-- ## GAIA MOD START
			local iLakeCandidates = table.maxn(trout_list);
			if iLakeCandidates > 0 then
				--print("Placing 'sexy Lake Fish' in third ring of start location in Region#", region_number);
				local shuf_list = GetShuffledCopyOfTable(trout_list)
				local iNumLeftToPlace = self:PlaceSpecificNumberOfResources(self.lakefish_ID, 1, 1, 1, ImpactLayers.LAYER_NONE, 0, 0, shuf_list);
			end
			-- ## GAIA MOD END
		end
	end
	...
	for y = 0, iH - 1 do
		for x = 0, iW - 1 do
			...
			elseif res_ID == self.cocoa_ID or 
				   ...
				   res_ID == self.berries_ID or -- GAIA
				   res_ID == self.timber_ID or 	-- GAIA
				   res_ID == self.rubber_ID 	-- GAIA
			...
			elseif res_ID == self.incense_ID or 
				   ...
				   --res_ID == self.roses_ID or 
				   res_ID == self.cotton_ID or
				   res_ID == self.poppies_ID
			then	
...
function AssignStartingPlots:PrintFinalResourceTotalsToLog()
	...
	print(self.poppies_ID,  "Poppies.: ", self.amounts_of_resources_placed[self.poppies_ID + 1])
	--print(self.roses_ID,  	"Roses...: ", self.amounts_of_resources_placed[self.roses_ID + 1])
	...
	--print(self.tulips_ID,	"Tulips: ", self.amounts_of_resources_placed[self.tulips_ID + 1])
	print("-")
	...
	print("- Mod BONUS Resources -")
	print(self.berries_ID,  "Berries.: ", self.amounts_of_resources_placed[self.berries_ID + 1])
	print(self.cactus_ID,  	"Cactus..: ", self.amounts_of_resources_placed[self.cactus_ID + 1])
	print(self.coconuts_ID, "Coconuts: ", self.amounts_of_resources_placed[self.coconuts_ID + 1])
	print(self.flax_ID,  	"Flax....: ", self.amounts_of_resources_placed[self.flax_ID + 1])
	print(self.lakefish_ID, "Lakefish: ", self.amounts_of_resources_placed[self.lakefish_ID + 1])
	...
	print(self.timber_ID, 	"Timber..: ", self.amounts_of_resources_placed[self.timber_ID + 1])
	print(self.tin_ID,      "Tin.....: ", self.amounts_of_resources_placed[self.tin_ID + 1])
	print(self.kelp_ID, 	"Kelp....: ", self.amounts_of_resources_placed[self.kelp_ID + 1])
	--print(self.minerals_ID, "Minerals: ", self.amounts_of_resources_placed[self.minerals_ID + 1])
	print("-----------------------------------------------------");
end
------------------------------------------------------------------------------
...
function AssignStartingPlots:PlaceBonusResources()
	...
	--self:PlaceMinerals(10 * resMultiplier, self.offshore_list)
	self:PlaceKelp(24 * resMultiplier, self.coast_list)
...
--BERRIES
	resources_to_place = {
	{self.berries_ID, 1, 100, 1, 2} };
	self:ProcessResourceList(10 * resMultiplier, ImpactLayers.LAYER_BONUS, self.marsh_list, resources_to_place)
	self:ProcessResourceList(65 * resMultiplier, ImpactLayers.LAYER_BONUS, self.forest_flat_that_are_not_tundra, resources_to_place)
	self:ProcessResourceList(44 * resMultiplier, ImpactLayers.LAYER_BONUS, self.tundra_flat_forest, resources_to_place)
	self:ProcessResourceList(22 * resMultiplier, ImpactLayers.LAYER_BONUS, self.tundra_flat_no_feature, resources_to_place)
	
--CACTUS
	resources_to_place = {
	{self.cactus_ID, 1, 100, 1, 2} };
	self:ProcessResourceList(12 * resMultiplier, ImpactLayers.LAYER_BONUS, self.desert_flat_no_feature, resources_to_place)

--COCONUT
	resources_to_place = {
	{self.coconuts_ID, 1, 100, 0, 2} };
	self:ProcessResourceList(24 * resMultiplier, ImpactLayers.LAYER_BONUS, self.coconut_list, resources_to_place)
	
--FLAX
	resources_to_place = {
	{self.flax_ID, 1, 100, 0, 2} };
	self:ProcessResourceList(22 * resMultiplier, ImpactLayers.LAYER_BONUS, self.flax_list, resources_to_place)

-- TIMBER
	resources_to_place = {
	{self.timber_ID, 1, 100, 1, 2} };
	self:ProcessResourceList(36 * resMultiplier, ImpactLayers.LAYER_BONUS, self.hills_covered_list, resources_to_place)
	self:ProcessResourceList(36 * resMultiplier, ImpactLayers.LAYER_BONUS, self.flat_covered, resources_to_place)
	self:ProcessResourceList(30 * resMultiplier, ImpactLayers.LAYER_BONUS, self.tundra_flat_forest, resources_to_place)

-- TIN
	resources_to_place = {
	{self.tin_ID, 1, 100, 1, 3} };
	self:ProcessResourceList(40 * resMultiplier, ImpactLayers.LAYER_BONUS, self.dry_grass_flat_no_feature, resources_to_place)

	resources_to_place = {
	{self.tin_ID, 1, 100, 2, 3} };
...
--TROUT
	resources_to_place = {
	{self.lakefish_ID, 1, 100, 1, 2} };
	self:ProcessResourceList(4 * resMultiplier, ImpactLayers.LAYER_BONUS, self.lake_list, resources_to_place)
...
function AssignStartingPlots:PlaceResourcesAndCityStates()
	...
	print("Map Generation - Generating global plot lists for resource distribution");
	...
	print("Map Generation - Placing Strategic and Bonus resources");
