-- GAIA: update to be the same as Great Plains XP (since the latter is just an update for GK and should really just have replaced the former instead of bein added as an additional map)
-------------------------------------------------------------------------------
...
function AddRivers()
	...
	for iPass, passCondition in ipairs(passConditions) do
		...
		local seaWaterRange;
		if (iPass <= 2) then
			riverSourceRange = math.max(2, GameDefines["RIVER_SOURCE_MIN_RIVER_RANGE"]) -- GAIA edit
			seaWaterRange = math.max(2, GameDefines["RIVER_SOURCE_MIN_SEAWATER_RANGE"]) -- GAIA edit
		else
			riverSourceRange = math.max(2, (GameDefines["RIVER_SOURCE_MIN_RIVER_RANGE"] / 2)) -- GAIA edit
			seaWaterRange = math.max(2, (GameDefines["RIVER_SOURCE_MIN_SEAWATER_RANGE"] / 2)) -- GAIA edit
		end
...
--CACTUS
	...
	self:ProcessResourceList(11 * resMultiplier, ImpactLayers.LAYER_BONUS, self.desert_flat_no_feature, resources_to_place)
